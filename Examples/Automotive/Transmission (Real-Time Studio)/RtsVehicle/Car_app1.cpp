// implementation for class Car_app1
#include "StdAfx.h"
...
void Car_app1::Fire_Tick(const double t)
{
...
HRESULT Car_app1::handle_tick(const double t)
{
  sim_time = t;
  printData(t);
...
void Car_app1::printData(const double t)
{
  long i;
...
double Car_app1::getSpeed(const double t)
{
  double val;
...
double Car_app1::getdSpeed(const double t)
{
  double val;
  // NOTE: could add a couple lines to get this variable by name, but this was much easier.
...
double Car_app1::getRPM(const double t)
{
  double val;
  // NOTE: could add a couple lines to get this variable by name, but this was much easier.
...
void Car_app1::set_throttle(const double t, double* val)
{
  //AcceleratorValue->setPosition(t, val);
  AcceleratorValue = *val;
}


void Car_app1::set_brake(const double t, double* val)
{
  //Brake->setPosition(t, val);
  BrakeValue = *val;
}

void Car_app1::runSimulation()
{
  HRESULT hr = S_OK;
...
#if _use_spring_damper
CarWithDriveTrain_handle_OBShift = new COnListener<Car1, long>(m_pUMLModel, &Car1::handle_OBShift);
CarWithDriveTrain_handle_OBShift->AddRef();
AODController->Port_7->Advise(CarWithDriveTrain_handle_OBShift);
CarWithDriveTrain_handle_DCShift = new COnListener<Car1, long>(m_pUMLModel, &Car1::handle_DCShift);
CarWithDriveTrain_handle_DCShift->AddRef();
AODController->Port_6->Advise(CarWithDriveTrain_handle_DCShift);
CarWithDriveTrain_handle_TCCShift = new COnListener<Car1, long>(m_pUMLModel, &Car1::handle_TCCShift);
CarWithDriveTrain_handle_TCCShift->AddRef();
//AODController->Port_5->Advise(CarWithDriveTrain_handle_TCCShift);
CarWithDriveTrain_handle_IOCShift = new COnListener<Car1, long>(m_pUMLModel, &Car1::handle_IOCShift);
CarWithDriveTrain_handle_IOCShift->AddRef();
AODController->Port_4->Advise(CarWithDriveTrain_handle_IOCShift);
CarWithDriveTrain_handle_POCShift = new COnListener<Car1, long>(m_pUMLModel, &Car1::handle_POCShift);
CarWithDriveTrain_handle_POCShift->AddRef();
AODController->Port_3->Advise(CarWithDriveTrain_handle_POCShift);
CarWithDriveTrain_handle_FCShift = new COnListener<Car1, long>(m_pUMLModel, &Car1::handle_FCShift);
CarWithDriveTrain_handle_FCShift->AddRef();
AODController->Port_2->Advise(CarWithDriveTrain_handle_FCShift);
CarWithDriveTrain_handle_LRBShift = new COnListener<Car1, long>(m_pUMLModel, &Car1::handle_LRBShift);
CarWithDriveTrain_handle_LRBShift->AddRef();
AODController->Port_1->Advise(CarWithDriveTrain_handle_LRBShift);
CarWithDriveTrain_handle_RCShift = new COnListener<Car1, long>(m_pUMLModel, &Car1::handle_RCShift);
CarWithDriveTrain_handle_RCShift->AddRef();
AODController->Port_0->Advise(CarWithDriveTrain_handle_RCShift);
CarWithDriveTrain_handle_brakeTorque = new COnListener<Car1, double>(m_pUMLModel, &Car1::handle_brakeTorque);
CarWithDriveTrain_handle_brakeTorque->AddRef();
//Brake->positionPort->Advise(CarWithDriveTrain_handle_brakeTorque);
CarWithDriveTrain_handle_throttle = new COnListener<Car1, double>(m_pUMLModel, &Car1::handle_throttle);
...
bool Car_app1::Load(BSTR filename)
{
...
void Car_app1::Save(BSTR filename)
{
  CComBSTR bstrName = filename;
  CComBSTR bstrControlName = filename;
  bstrName.Append(L".bin");
...
void Car_app1::ReverseControllerShiftTiming()
{
  AODController->bReverse = true;
}

double Car_app1::get_LastTime()
{
  double val;
...
double Car_app1::get_AccelerationDot(const double t)
{
  double val;
  // NOTE: could add a couple lines to get this variable by name, but this was much easier.
...
double Car_app1::get_Acceleration(const double t)
{
  double val;
  // NOTE: could add a couple lines to get this variable by name, but this was much easier.
...
void Car_app1::put_ThrottlePosition(const double t, double* val)
{
  //Accelerator->setPosition(t, val);
  AcceleratorValue = *val;
}

void Car_app1::put_BrakePosition(const double t, double* val)
{
  //Brake->setPosition(t, val);
  BrakeValue = *val;
}

void Car_app1::put_shiftTiming(bool bUpshift, long iIndex, double val)
{
  if (bUpshift)
