// implementation for class demotest_app
#include "StdAfx.h"
#include "RtsOnTransmission.h"
#include <math.h>

#include "demotest_app.h"
// includes
#include "Throttle.h"
...
void demotest_app::Fire_Tick(const double t)
{
...
void demotest_app::initSimulation()
{
...
void demotest_app::runSimulation()
{
...
HRESULT demotest_app::handle_tick(const double t)
{
...
double val = 90.0;
if (sim_time >= 6.0 && aThrottle->currentPosition != val)  
  aThrottle->setPosition(sim_time, &val);
...
void demotest_app::termSimulation()
{
...
bool demotest_app::Load(const BSTR filename)
{
...
void demotest_app::Save(const BSTR filename)
{
...
m_pModel->get_Eso(L"gIOC", &m_gIOC);
m_pModel->get_Eso(L"aIOC", &m_aIOC);
...
aIOC_handle_shiftEvent = new COnListener<Actuator, long>(m_pUMLModel->aIOC, &Actuator::handle_shiftEvent);
aIOC_handle_shiftEvent->AddRef();
aController->ClutchPort7->Advise(aIOC_handle_shiftEvent);
...
if (aIOC_handle_shiftEvent)
{
  aIOC_handle_shiftEvent->clean();
  aIOC_handle_shiftEvent->Release();
  aIOC_handle_shiftEvent = NULL;
}
