// DistributedVar.cpp: implementation of the DistributedVar class.
//
//////////////////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////////////////
DistributedVar::DistributedVar(char* VarName,
								char* VarUnits,
								long VarnDomains,
								DistributionDomain* VarDomains,
								DistributedVar* Parent
								)
{
	long i; 
...
DistributedVar::init(DistributedVar& Src)
{
	long i; 
...
// Copy constructor -- 
DistributedVar::DistributedVar(DistributedVar& Src){
	init(Src);
}
...
void DistributedVar::mesh(bool bInitVal){
	...
	pVal = new double[nVal];
	if(bInitVal) for(i=0;i<nVal;i++) pVal[i] = 0.0; // or randomize

}

void DistributedVar::setData(long index, double pData)
{
	if(bIsRoot)
...
void DistributedVar::copyData(DistributedVar* pSrc) // make private
{
	long i;
	bIsRoot = true;
	pParent = NULL;
...
double DistributedVar::getVal(double* domainPoint){
	long index[8], localIndex[3];
	// DistributedVar* Src;
	double temp, lambda[3];
	...
	case 1:
		...
		index[0] = (long) floor(temp);
		index[1] = (long) ceil(temp);
	...
	case 2:
		...
		index[2] = (long) floor(temp);
		index[3] = (long) ceil(temp);
...
DistributedVar::operator = (DistributedVar& Src){

	this->init(Src);
	this->pParent = &Src;
	// while((this->pParent)->pParent)
	//	(this->pParent)=(this->pParent)->pParent;

}

DistributedVar DistributedVar::operator | (DistributionDomain& AtSlice){
	long i,j;
	...
	DistributedVar* pVar;
	pVar = new DistributedVar(this->Name,this->Units,this->nDomains,this->Domains, this);  // (*this);
	pVar->nVal = 1;
	for(i=0;i<nDomains;i++){
	...
	// pVar->pParent = this;
	// while((pVar->pParent)->pParent)
	//	(pVar->pParent)=(pVar->pParent)->pParent;
...
DistributedVar DistributedVar::operator + (DistributedVar& opVar)
{
	long i;

	DistributedVar* pVar;
	pVar = new DistributedVar;
	(*pVar) = (*this);
...
DistributedVar DistributedVar::operator + ()
{
...
DistributedVar DistributedVar::operator * (double opConst)
{
	long i;
	DistributedVar* pVar;
	pVar = new DistributedVar;
...
DistributedVar& DistributedVar::PARTIAL(DistributionDomain& partialDomain) 
{
...
long DistributedVar::getGlobalIndex (long* gridIndex){
...
void DistributedVar::getLocalIndex (long index, long* &gridIndex){
...
bool DistributedVar::isCompatible(DistributedVar& otherVar, long* index){
...
bool DistributedVar::inDomain(long* index){
