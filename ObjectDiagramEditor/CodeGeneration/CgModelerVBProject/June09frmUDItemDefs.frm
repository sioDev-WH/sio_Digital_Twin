...
Private Sub btnCollapse_Click()
    ...
    If btnCollapse.Caption = "<" Then
        ...
        btnCollapse.Caption = ">"
    Else
        frmDetails.Height = 3750
        Me.Height = frmMasks.Height * 2 + 510
    ...
    While i <= m_collLibraryShapes.count
        ...
        If m_collLibraryShapes(i).bModified Then
            ...
            If bDoChange Then
                ...
                Set pDef = m_UDIDefMgr.GetUDIDefbyFolderAndShape(cgShapeInfo.strFolderName, cgShapeInfo.strShapeName)
                If cgShapeInfo.bConnectorsUpdated And Not m_collLibraryShapes(i).bAdded Then
                    ...
                    If Not pDef Is Nothing Then
                        If Not UpdateShape(cgShapeInfo.strShapeName, cgShapeInfo.strFolderName, strFolderID, m_GeLbrMgr, cgShapeInfo) Then
                           gMsgWnd.LogMessage "Failed to auto generate library block '" & pDef.GeShape & "'."
                        Else
                           pDef.GetConnectionDetails pDef.GeLibrary, pDef.GeFolder, pDef.GeShape    'Fill In Face and Order and UserID since it may have changed
                        End If
                    End If
                ...
                ElseIf Not pDefOriginal Is Nothing Then
                    Set pDef = m_UDIDefMgr.GetUDIDefbyObjectName(pDefOriginal.ObjectName)
            ...
            Else
                ...
                If Not cgShapeInfo.m_UDIDefOriginal Is Nothing Then
                   ...
                   Set pDefOriginal = cgShapeInfo.m_UDIDefOriginal
                   Set pDefCurrent = m_UDIDefMgr.GetUDIDefbyObjectName(pDefOriginal.ObjectName)
               ...
               If pDefCurrent Is Nothing Then Set pDefCurrent = m_UDIDefMgr.GetUDIDefbyFolderAndShape(cgShapeInfo.strFolderName, cgShapeInfo.strShapeName)
...
Private Sub btnCancel_Click()
    ...
    For i = 1 To m_collLibraryShapes.count
        ...
        If m_collLibraryShapes(i).bModified Then
           ...
           'Restore Connections
           Set pDefCurrent = m_UDIDefMgr.GetUDIDefbyFolderAndShape(cgShapeInfo.strFolderName, cgShapeInfo.strShapeName)
           If Not pDefCurrent Is Nothing Then
                For n = 1 To pDefCurrent.CgConnectionDefs.count
                    pDefCurrent.CgConnectionDefs.Remove 1
                Next n
...
Private Sub AddRenamedShapeToCollection(strShapeNew As String)
    ...
    'Case where a shape by the same name has already been deleted could exist.  In that case restore it
    Set cgShapeInfoDeleted = m_collLibraryShapes(strKey)
    ...
    Else
        ...
        nRet = MsgBox("Restore old shape " & strShapeNew & "and assigned connectors?", vbYesNo)
        If nRet = vbNo Then
            For i = 1 To cgShapeInfoDeleted.m_collConnAssignment.count
                cgShapeInfoDeleted.m_collConnAssignment.Remove 1
                cgShapeInfoDeleted.m_collConnectors.Remove 1
            ...
            If Not cgShapeInfoDeleted.m_UDIDefOriginal Is Nothing Then
                ...
                Next i
            End If
            Set pDef = m_UDIDefMgr.GetUDIDefbyFolderAndShape(m_currShapeFolder, strShapeNew)
            If Not pDef Is Nothing Then
...
Private Sub AddModifiedConnectorMapping(strAttrib As String, strConOld As String, strConNew As String)
    ...
    If cgShapeInfo.m_UDIDefOriginal Is Nothing Then
        ...
        Set pDef = m_UDIDefMgr.GetUDIDefbyFolderAndShape(m_currShapeFolder, m_currShape)
        Set cgShapeInfo.m_UDIDefOriginal = New UserDefinedItemDef
        If Not pDef Is Nothing Then pDef.Copy cgShapeInfo.m_UDIDefOriginal
    End If
...
Private Sub DoInit()
  ...
  ShapeGe.Grid = False
  g_DrawingDefMgr.m_custDrawEditor.GeCtrl.SelRemoveAll
  ...
  End If
'  SetGeCtrl g_DrawingDefMgr.m_custDrawEditor.GeCtrl
...
Private Sub InitLibraryLists()
  ...
  Else                                      'Launched from a shape instance...only that shape will be included
    ...
    On Error Resume Next
    Set pDef = m_UDIDefMgr.GetUDIDefbyFolderAndShape(m_strStartupFolder, m_strStartupShape)
    
    On Error GoTo Error_Handler
    If Not pDef Is Nothing Then
        Set m_curUDIDef = pDef
        Set m_curNode = aNode
        If Not ShapeExists(GetFolderID(), pDef.GeShape, m_GeLbrMgr) Then AddNewShapeToCollection pDef
    End If
  End If
...
Private Sub AssignShapeToCurItemDef(bAssign As Boolean)
    ...
    If cgShapeInfo Is Nothing Then
        ...
        Exit Sub
    End If
    Set UDIDef = m_UDIDefMgr.GetUDIDefbyFolderAndShape(m_currShapeFolder, m_currShape)
    If Not UDIDef Is Nothing Then
      If bAssign Then
        ...
        If GetClassInfo(aProject, UDIDef.packageName, UDIDef.ClassName, aCI, True) Then
...
Private Sub DoObjectDefEditAttributes()
  ...
  On Error Resume Next
  If Not m_UDIDefMgr Is Nothing Then
    Set objDef = m_UDIDefMgr.GetUDIDefbyFolderAndShape(m_currShapeFolder, m_currShape)
...
Private Sub tvwShapes_NodeClick(ByVal Node As MSComctlLib.Node)
  ...
  If GetNodeFolderAndShapeName(Node, FolderName, ShapeName) Then
    ...
    If FolderName = "" Or ShapeName = "" Then Exit Sub
   ...
   'See if there's a UDIDef for this ShapeDef
    Set UDIDef = m_UDIDefMgr.GetUDIDefbyFolderAndShape(FolderName, ShapeName)
    If Not UDIDef Is Nothing Then
        UDIDef.GetConnectionDetails UDIDef.GeLibrary, UDIDef.GeFolder, UDIDef.GeShape     'Fill In face and Order
...
Private Sub RefreshConnectorGrid()
    ...
    If Not m_curUDIDef Is Nothing Then
        ...
        If Not m_ConnectionInfo Is Nothing Then
            For Each aConn In m_ConnectionInfo
                If Not cgConnDetail Is Nothing Then iFace = cgConnDetail.m_collFace(aConn.connName)
                If Not cgConnDetail Is Nothing Then iOrder = cgConnDetail.m_collOrder(aConn.connName)
    ...
    Else
        ...
        For Each aInfo In m_ConnectionInfo
            gdConnectors.Rows.Add sgFormatCharSeparatedValue, aInfo.NameA & "," & 1 & "," & 1
            SetModifiedConnectorInfo "", aInfo.NameA, 1, 1, True
        Next
    End If
End Sub
      ...
      If Len(aNode.Key) > 2 Then                        ' a leaf node key
        ...
        If szKeyType = IO_KEY_PREFIX Or szKeyType = CN_KEY_PREFIX Or szKeyType = VR_KEY_PREFIX Or szKeyType = BR_KEY_PREFIX Or szKeyType = LI_KEY_PREFIX Then
          ...
          If aNode.Selected Then
            ...
            On Error Resume Next
            UpdateUDIDef szConnector, szConnector, aAttr, Nothing
            aAttr.ConnId = szConnector
          ElseIf aAttr.ConnId = szConnector Then
        ...
        ElseIf szKeyType = OP_KEY_PREFIX Then
          ...
          If aNode.Selected Then
            ...
            UpdateUDIDef szConnector, szConnector, Nothing, aOper
            aOper.ConnId = szConnector
          ElseIf aOper.ConnId = szConnector Then
...
Private Sub FillClassItems()
  ...
  'If ClassVariables Is Nothing Then
    GetClassVariables SelectedClass, False, "", ClassVariables
  'End If
...
Sub InitConnectionData()
  ...
  'Clear out all the connectors for the selected class if same class
  If SelectedClassOld.Name = SelectedClass.Name Then
...
Public Function UpdateShape(strShape As String, strFolder As String, strFolderID As String, m_GeLbrMgr As GeLibraryMgr, cgDetailInfo As cgLibraryShapeDetails) As Boolean
    ...
    End If
    'For now, if a conenctor is added or modified, just remove all the old ones, and regen the current ones
    'If collAdd.count > 0 Then AddConnectors collAdd, cgDetailInfo, m_GeLbrMgr
    AddConnectors collAdd, cgDetailInfo, m_GeLbrMgr     'Note: Right now collAdd isn't used
