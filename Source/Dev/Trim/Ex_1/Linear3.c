   ...
   INTARG vartwo)
{
   ...
   lhs[2] = y[0]; /* constraint left hand side */
   lhs[3] = y[1]; /* constraint left hand side */
   return 0;
}
   ...
   LPBYTEARG objtype, LPBYTEARG matvaltype)
{
   ...
   matval[9] = 0;
   matval[13] = -2;
      /* Partial derivatives of y[0] */
   matval[2] = 0;
   matval[6] = 2;
   matval[10] = -3;
   matval[14] = 0;
   /* Partial derivatives of y[1] */
   matval[3] = -1;
   matval[7] = -5;
   matval[11] = 1;
   matval[15] = 0;
...
void linear(void)
{
   ...
   lp = loadnlp (PROBNAME, 4, 4, 1, obj, rhs, sense,
      NULL, NULL, NULL, matval, x, lb, ub, NULL, 16,
      trimObj, jacobian1);
   if (!lp) return;
