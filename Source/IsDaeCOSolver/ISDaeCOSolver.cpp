// ISDaeCOSolver.cpp : Implementation of DLL Exports.


// Note: Proxy/Stub Information
//      To build a separate proxy/stub DLL, 
//      run nmake -f ISDaeCOSolverps.mk in the project directory.
...
//OBJECT_ENTRY(CLSID_IsDaeSolver, CIsDaeSolver)
OBJECT_ENTRY(CLSID_IsMatrix, CIsMatrix)
OBJECT_ENTRY(CLSID_IsNlaSolver, CIsNlaSolver)
OBJECT_ENTRY(CLSID_IsCINlaEso, CIsCINlaEso)
OBJECT_ENTRY(CLSID_IsDaeEsoIImpl, CIsDaeEsoIImpl)
OBJECT_ENTRY(CLSID_IsDaeEsoCImpl, CIsDaeEsoCImpl)
OBJECT_ENTRY(CLSID_OnModelExec, COnModelExec)
OBJECT_ENTRY(CLSID_OnTest, COnTest)
OBJECT_ENTRY(CLSID_OnEnv, COnEnv)
END_OBJECT_MAP()
...
BOOL WINAPI DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID /*lpReserved*/)
{
    ...
    {
        _Module.Init(ObjectMap, hInstance, &LIBID_ISDAECOSOLVERLib);
...
//       save      first, eps, sfmin, base, t, rnd, emin, rmin,emax, rmax, prec (could be static vars?)
      ...
      if( first ){
         ...
         if( fsmall >= sfmin ){
//
//           use fsmall plus a bit, to avoid the possibility of rounding
//           causing overflow when computing  1/sfmin.
//
            sfmin = fsmall*( one+eps );
         }
      }
//
...
//     .. local scalars ..
      ...
      double		  sixth, fsmall, third, two, zero;
...
//
         ...
         fsmall = one;
         for(i =0;i<3;i++)
            fsmall = dlamc3( fsmall*rbase, zero );
