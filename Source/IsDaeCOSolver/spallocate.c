...
register  AllocationListPtr  ListPtr, NextListPtr, temp;
 ...
 * along the way. */
    ...
    */
    while (ListPtr != NULL)
    {   NextListPtr = ListPtr->NextRecord;
        if( ((char *)ListPtr->AllocatedPtr != (char *)Matrix) && ((char *)ListPtr->AllocatedPtr != (char *)ListPtr)){
				  FREE( ListPtr->AllocatedPtr );
        }
        else {
          if(((char *)ListPtr->AllocatedPtr == (char *)ListPtr)) temp = (char *) ListPtr;
