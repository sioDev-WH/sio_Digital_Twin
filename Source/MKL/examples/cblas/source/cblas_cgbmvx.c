...
void GetInputData(FILE*, int*, int*, int*, int*, int*, int*, MKL_Complex8*, MKL_Complex8*,
...
/*       Get input data                                       */
      ...
      GetInputData(in_file, &m, &n, &kl, &ku, &incx, &incy, &alpha, &beta, &trans, &order);
      ...
      GetBandArrayC(in_file, &order, kl, ku, m, n, &a[0][0], lda);
      fclose(in_file);
...
/*       Print input data                                      */
      ...
      PrintBandArrayC(&order, 0, kl, ku, m, n, &a[0][0], lda, "A");

/*      Call CBLAS_CGBMV subroutine ( C Interface )                  */
      cblas_cgbmv(order, trans, m, n, kl, ku, &alpha, &a[0][0], lda, 
                  x, incx, &beta, y, incy);
...
void GetInputData(FILE *in_file, int *m, int *n, int *kl, int *ku, 
                  int *incx, int *incy, MKL_Complex8 *alpha, MKL_Complex8 *beta,
