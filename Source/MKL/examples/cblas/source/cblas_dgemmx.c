...
void main(int argc, char *argv[])
{
      ...
      int             lda, ldb, ldc;
      double          alpha, beta;
      double          a[rmaxa][cmaxa], b[rmaxb][cmaxb], c[rmaxc][cmaxc];
      CBLAS_ORDER     order;
...
/*       Get input data                                       */
      ...
      if (transA == CblasNoTrans)
         GetArrayD(in_file, &order, 0, &m, &k, &a[0][0],  &lda);
      else
         GetArrayD(in_file, &order, 0, &k, &m, &a[0][0], &lda);
...
/*       Print input data                                      */
      ...
      PrintOrder(&order);
      if (transA == CblasNoTrans)
         PrintArrayD(&order, 0, 0, &m, &k, &a[0][0], &lda, "A");
      else
         PrintArrayD(&order, 0, 0, &k, &m, &a[0][0], &lda, "A");
      if (transB == CblasNoTrans)
