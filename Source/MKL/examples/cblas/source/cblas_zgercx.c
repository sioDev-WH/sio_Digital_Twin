...
void GetInputData(FILE*, int*, int*, int*, int*, MKL_Complex16*, CBLAS_ORDER*);
...
/*      Call CBLAS_ZGERC subroutine ( C Interface )                  */
      cblas_zgerc(order, m, n, &alpha, x, incx, y, incy,
                 &a[0][0], lda);
                                                             
/*       Print output data                                      */
      printf("\n\n     OUTPUT DATA");
      PrintArrayZ(&order, 1, 0, &m, &n, &a[0][0], &lda, "A");
end:  return;
}

void GetInputData(FILE *in_file, int *m, int *n, int *incx, int *incy, 
                  MKL_Complex16 *alpha, CBLAS_ORDER *order)
{
