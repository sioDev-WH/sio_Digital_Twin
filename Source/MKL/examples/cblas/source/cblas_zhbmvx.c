...
/*       Get input data                                       */
      ...
      if (uplo == CblasUpper) 
        GetBandArrayZ(in_file, &order, 0, k, n, n, &a[0][0], lda);
      else
        GetBandArrayZ(in_file, &order, k, 0, n, n, &a[0][0], lda);
      fclose(in_file);
...
/*       Print input data                                      */
      ...
      if (uplo == CblasUpper) 
        PrintBandArrayZ(&order, 1, 0, k, n, n, &a[0][0], lda, "A");
      else
        PrintBandArrayZ(&order, 1, k, 0, n, n, &a[0][0], lda, "A");

/*      Call CBLAS_ZHBMV subroutine ( C Interface )                  */
      cblas_zhbmv(order, uplo, n, k, &alpha, &a[0][0], lda, x, incx,
                  &beta, y, incy);
