...
int main()
{
  ...
  for(i=0;i<vec_len;i++) {
    dA1[i]=(double)(__VML_DDIV_BEG+((__VML_DDIV_END-__VML_DDIV_BEG)*i)/vec_len);
    dA2[i]=(double)(__VML_DDIV_BEG+((__VML_DDIV_END-__VML_DDIV_BEG)*i)/vec_len);
    dB1[i]=0;
    dB2[i]=(dA1[i]/dA2[i]);
  }

#if defined(MKL_VML_MIXED)
  vdDiv(vec_len,dA1,dA2,dB1);
...
# error You must define MKL_VML_CDECL for CDECL call convention or MKL_VML_STDCALL for STDCALL call convention.
  ...
  printf("vdDiv test/example programm\n\n");
  printf("           Arguments                    vdDiv                      x/y\n");
  printf("===============================================================================\n");
  for(i=0;i<vec_len;i++) {
