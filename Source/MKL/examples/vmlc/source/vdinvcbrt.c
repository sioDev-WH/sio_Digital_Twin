...
int main()
{
  ...
  for(i=0;i<vec_len;i++) {
    dA[i]=(double)(__VML_DINVCBRT_BEG+((__VML_DINVCBRT_END-__VML_DINVCBRT_BEG)*i)/vec_len);
    dB1[i]=0;
    dB2[i]=1.0/pow((dA[i]),1.0/3.0);
  }

#if defined(MKL_VML_MIXED)
  vdInvCbrt(vec_len,dA,dB1);
...
# error You must define MKL_VML_CDECL for CDECL call convention or MKL_VML_STDCALL for STDCALL call convention.
  ...
  printf("vdInvCbrt test/example programm\n\n");
  printf("           Argument                     vdInvCbrt            1.0/pow(x,1.0/3.0)\n");
  printf("===============================================================================\n");
  for(i=0;i<vec_len;i++) {
