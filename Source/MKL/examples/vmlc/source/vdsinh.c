...
int main()
{
  ...
  for(i=0;i<vec_len;i++) {
    dA[i]=(double)(__VML_DSINH_BEG+((__VML_DSINH_END-__VML_DSINH_BEG)*i)/vec_len);
    dB1[i]=0;
    dB2[i]=sinh(dA[i]);
  }

#if defined(MKL_VML_MIXED)
  vdSinh(vec_len,dA,dB1);
...
# error You must define MKL_VML_CDECL for CDECL call convention or MKL_VML_STDCALL for STDCALL call convention.
  ...
  printf("vdSinh test/example programm\n\n");
  printf("           Argument                     vdSinh                     Sinh\n");
  printf("===============================================================================\n");
  for(i=0;i<vec_len;i++) {
