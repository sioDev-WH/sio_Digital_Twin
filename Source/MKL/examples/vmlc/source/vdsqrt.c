...
int main()
{
  ...
  for(i=0;i<vec_len;i++) {
    dA[i]=(double)(__VML_DSQRT_BEG+((__VML_DSQRT_END-__VML_DSQRT_BEG)*i)/vec_len);
    dB1[i]=0;
    dB2[i]=sqrt(dA[i]);
  }

#if defined(MKL_VML_MIXED)
  vdSqrt(vec_len,dA,dB1);
...
# error You must define MKL_VML_CDECL for CDECL call convention or MKL_VML_STDCALL for STDCALL call convention.
  ...
  printf("vdSqrt test/example programm\n\n");
  printf("           Argument                     vdSqrt                     Sqrt\n");
  printf("===============================================================================\n");
  for(i=0;i<vec_len;i++) {
