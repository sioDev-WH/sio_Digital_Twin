...
int main()
{
  ...
  for(i=0;i<vec_len;i++) {
    fA[i]=(float)(__VML_SASIN_BEG+((__VML_SASIN_END-__VML_SASIN_BEG)*i)/vec_len);
    fB1[i]=0;
    fB2[i]=(float)asin(fA[i]);
  }

#if defined(MKL_VML_MIXED)
  vsAsin(vec_len,fA,fB1);
...
# error You must define MKL_VML_CDECL for CDECL call convention or MKL_VML_STDCALL for STDCALL call convention.
  ...
  printf("vsAsin test/example programm\n\n");
  printf("           Argument                     vsAsin                      Asinf\n");
  printf("===============================================================================\n");
  for(i=0;i<vec_len;i++) {
