...
int main()
{
  ...
  for(i=0;i<vec_len;i++) {
    fA[i]=(float)(__VML_SCOSH_BEG+((__VML_SCOSH_END-__VML_SCOSH_BEG)*i)/vec_len);
    fB1[i]=0;
    fB2[i]=(float)cosh(fA[i]);
  }

#if defined(MKL_VML_MIXED)
  vsCosh(vec_len,fA,fB1);
...
# error You must define MKL_VML_CDECL for CDECL call convention or MKL_VML_STDCALL for STDCALL call convention.
  ...
  printf("vsCosh test/example programm\n\n");
  printf("           Argument                     vsCosh                     Coshf\n");
  printf("===============================================================================\n");
  for(i=0;i<vec_len;i++) {
