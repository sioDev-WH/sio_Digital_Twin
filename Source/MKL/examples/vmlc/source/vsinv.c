...
int main()
{
  ...
  for(i=0;i<vec_len;i++) {
    fA[i]=(float)(__VML_SINV_BEG+((__VML_SINV_END-__VML_SINV_BEG)*i)/vec_len);
    fB1[i]=0;
    fB2[i]=(float)(1.0/fA[i]);
  }

#if defined(MKL_VML_MIXED)
  vsInv(vec_len,fA,fB1);
...
# error You must define MKL_VML_CDECL for CDECL call convention or MKL_VML_STDCALL for STDCALL call convention.
  ...
  printf("vsInv test/example programm\n\n");
  printf("           Argument                     vsInv                   1.0/x          \n");
  printf("===============================================================================\n");
  for(i=0;i<vec_len;i++) {
