...
int main()
{
  ...
  for(i=0;i<vec_len;i++) {
    fA[i]=(float)(__VML_SLOG10_BEG+((__VML_SLOG10_END-__VML_SLOG10_BEG)*i)/vec_len);
    fB1[i]=0;
    fB2[i]=(float)log10(fA[i]);
  }

#if defined(MKL_VML_MIXED)
  vsLog10(vec_len,fA,fB1);
...
# error You must define MKL_VML_CDECL for CDECL call convention or MKL_VML_STDCALL for STDCALL call convention.
  ...
  printf("vsLog10 test/example programm\n\n");
  printf("           Argument                     vsLog10                    Log10f\n");
  printf("===============================================================================\n");
  for(i=0;i<vec_len;i++) {
