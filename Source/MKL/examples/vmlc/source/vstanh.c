...
int main()
{
  ...
  for(i=0;i<vec_len;i++) {
    fA[i]=(float)(__VML_STANH_BEG+((__VML_STANH_END-__VML_STANH_BEG)*i)/vec_len);
    fB1[i]=0;
    fB2[i]=(float)tanh(fA[i]);
  }

#if defined(MKL_VML_MIXED)
  vsTanh(vec_len,fA,fB1);
...
# error You must define MKL_VML_CDECL for CDECL call convention or MKL_VML_STDCALL for STDCALL call convention.
  ...
  printf("vsTanh test/example programm\n\n");
  printf("           Argument                     vsTanh                     Tanhf\n");
  printf("===============================================================================\n");
  for(i=0;i<vec_len;i++) {
