...
*     .. Executable Statements ..
      CALL ZNANCHK('ZLEV1','VRON', 1, LEN, 1, CCOMP)
      DO 20 I = 1, LEN
...
99997 FORMAT (1X,I4,I3,3I5,2I36,I12)
      ...
      SUBROUTINE ZNANCHK(NAME, OPTS, M, N, LDX, X)
      CHARACTER   NAME*(*), OPTS(4)
      INTEGER     M, N, LDX
      COMPLEX*16  X(LDX, *)
      EXTERNAL    ZISNAN
      LOGICAL     ZISNAN
      CHARACTER*6 OBJECT, INOUT
      INTEGER I, J, MM, NN
      IF (OPTS(1).EQ.'m'.OR.OPTS(1).EQ.'M') THEN
         OBJECT = 'MATRIX'
      ELSE IF (OPTS(1).EQ.'v'.OR.OPTS(1).EQ.'V') THEN
         OBJECT = 'VECTOR'
      ELSE
         OBJECT = '?UNKNW'
      END IF
      IF (OPTS(3).EQ.'i'.OR.OPTS(3).EQ.'I') THEN
         INOUT = 'INPUT'
      ELSE IF (OPTS(3).EQ.'o'.OR.OPTS(3).EQ.'O') THEN
         INOUT = 'OUTPUT'
      ELSE
         INOUT = '?UNKNW'
      END IF
      IF (OPTS(4).EQ.'n'.OR.OPTS(4).EQ.'N') THEN
         MM = M
         NN = N
      ELSE
         MM = N
      ...
      DO J = 1, NN
         DO I = 1, MM
            IF (ZISNAN(X(I,J))) THEN
               WRITE(6,9991) NAME,OBJECT,INOUT,OPTS(2),OPTS(4),J,I
*               STOP '*** !!! INF OR NAN FOUND'
               WRITE(6, FMT=*) '*** !!! INF OR NAN FOUND'
            END IF
         END DO
      END DO
      RETURN
9991  FORMAT(1X,A6,': ON ',A6,1X,A6,1X,A1,' TRANSPOSE CODE: ',A1,
     +   ' ROW ',I6,' COLUMN ',I6)
      END

      LOGICAL FUNCTION ZISNAN( X )
      COMPLEX*16         X
...
*     .. Executable Statements ..
     ...
     $.OR.( IAND( IW( HGHWRD, 2 ), HGHINF ).EQ.HGHINF ) ) ZISNAN=.TRUE.
      RETURN
      END
...
*     imaginary and in real part of a COMPLEX*16 value
*     and
...
*     .. Executable Statements ..
      ...
      W = X
      IF( ( IAND( IW( HGHWRD, 1 ), HGHINF ).EQ.HGHINF )
     $   .OR.( IAND( IW( HGHWRD, 2 ), HGHINF ).EQ.HGHINF ) ) X = FUNNY
      RETURN
*
