/************************************************************************
*  LongInterval.C (Implementation of type LONGINTERVAL)
*
*  Version:                     1.11
*  Date:                        11/10/94
*  Author:                      O. Knueppel
...
static char sccs_id[] = "@(#)LongInterval.C 1.11 94/11/10 OK";
...
LONGINTERVAL::LONGINTERVAL (INTERVAL & x)
{
  LONGREAL linf, lsup;
...
LONGINTERVAL::LONGINTERVAL (LONGREAL & x)
{
  nDigits = GetBaseDigits ();
  nGuards = MIN_GUARDS;
  value = LiNewTemp (nDigits, nGuards);
...
LONGINTERVAL::LONGINTERVAL (LONGINTERVAL & x)
{
#ifdef __DONTCOPY__
  isTemporary = 1;
  nDigits = GetBaseDigits ();
...
LONGINTERVAL::LONGINTERVAL (PCHAR sz, INT n)
{
#ifdef __DONTCOPY__
  isTemporary = 0;
#endif
...
LONGINTERVAL & LONGINTERVAL::operator = (INTERVAL & x)
{
  LONGREAL linf, lsup;
...
LONGINTERVAL & LONGINTERVAL::operator = (LONGINTERVAL & x)
{
#ifdef __DONTCOPY__
...
INTERVAL RoundToInterval (LONGINTERVAL & x)
{
  DOUBLE d1, d2;
  LONGREAL y;
  y = Inf(x);
...
LONGINTERVAL operator - (LONGINTERVAL & x)
{
  LONGINTERVAL y, z;
  MakeTemporary (y);
  MakeTemporary (z);
...
LONGINTERVAL operator + (LONGINTERVAL & a, REAL r)
{
  LONGINTERVAL b, c(2);
  b = r;
  MakeTemporary (c);
...
LONGINTERVAL operator + (LONGINTERVAL & a, INTERVAL & iv)
{
  LONGINTERVAL b, c(2);
  b = iv;
  MakeTemporary (c);
...
LONGINTERVAL operator + (REAL r, LONGINTERVAL & a)
{
  LONGINTERVAL b, c(2);
  b = r;
  MakeTemporary (c);
...
LONGINTERVAL operator + (INTERVAL & iv, LONGINTERVAL & a)
{
  LONGINTERVAL b, c(2);
  b = iv;
  MakeTemporary (c);
...
LONGINTERVAL operator + (LONGINTERVAL & a, LONGINTERVAL & b)
{
  LONGINTERVAL c(2);
  MakeTemporary (c);
  LiAdd (Value(a), Value(b), Value(c), 0);
...
ostream & operator << (ostream & os, LONGINTERVAL & x)
{
  LiToOutput (os, Value(x));
  return os;
}

VOID StringToLi (LIREAL li, PCHAR sz)
...
// NOTE: li must have at least 16 guard digits
{
  ...
  lisztoli (sz, lrstr, li, wrk1, wrk2, wrk3, wrk4, wrk5);
...
LONGINTERVAL LongIntervalEnclosure (PCHAR sz)
{
  LONGINTERVAL x(16);
...
VOID Accumulate (LONGINTERVAL & x, VECTOR & a, VECTOR & b)
{
  INT dim = Dimension (a);
  INT g;
...
VOID Accumulate (LONGINTERVAL & x, VECTOR & a, INTERVAL_VECTOR & b)
{
  INT dim = Dimension (a);
  INT g;
  LIREAL liaccu1, liaccu2;
...
VOID Accumulate (LONGINTERVAL & x, INTERVAL_VECTOR & a, VECTOR & b)
{
  INT dim = Dimension (a);
  INT g;
  LIREAL liaccu1, liaccu2;
...
VOID Accumulate (LONGINTERVAL & x, INTERVAL_VECTOR & a, INTERVAL_VECTOR & b)
{
  INT dim = Dimension (a);
  INT g;
  LIREAL liaccu1, liaccu2;
