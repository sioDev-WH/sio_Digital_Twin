...
{
  init(VarName, VarUnits, VarnDomains, VarDomains);
}

void DistributedVar::init(char* VarName,
								char* VarUnits,
								long VarnDomains,
								DistributionDomain* VarDomains)
{
...
void DistributedVar::setParent(DistributedVar& src)
{
  pParent = src.pParent;
}

// Used in case of late construction (e.g., arrays of same source var)
void DistributedVar::init(DistributedVar& Src)
{
  long i; 
...
DistributedVar& DistributedVar::operator | (DistributionDomain& AtSlice){
	...
	DistributedVar* pVar;
	pVar = new DistributedVar(this->Name,this->Units,this->nDomains,this->Domains);  // (*this);
	pVar->nVal = 1;
	for(i=0;i<nDomains;i++){
