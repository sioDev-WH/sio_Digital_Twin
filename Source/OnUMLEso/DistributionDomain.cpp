										...
										bool bOpenDomainMax)
{
	...
	bOpenMax = false;
	mesh = NULL;
  init(DomainName, DomainMin, DomainMax, bOpenDomainMin, bOpenDomainMax);
}

void DistributionDomain::init(	const char* DomainName,
										double DomainMin,
										double DomainMax,
										bool bOpenDomainMin,
										bool bOpenDomainMax)
{
  ...
  Name[0] = '\0';
	...
	if (DomainName) strcpy(Name,DomainName);
	Min = DomainMin;
	Max = DomainMax;
...
DistributionDomain::operator =(DistributionDomain& Src){
  ...
  Name[0] = '\0';
	...
	if (Src.Name) strcpy(this->Name,Src.Name);
	this->Min = Src.Min;
	this->Max = Src.Max;
